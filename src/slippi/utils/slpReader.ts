...
function readRef(ref: SlpRefType, buffer: Uint8Array, offset: number, length: number, position: number): number {
  switch (ref.source) {
    ...
    case SlpInputSource.BUFFER:
      if (position >= ref.buffer.byteLength) {
        return 0;
      }
      
      //"ref" is source
      // "buffer" is target
      // offset is start for "buffer" (target)
      // position is start for "ref" (source)
      // length is number of bytes to copy

      var sourceBuffer: ArrayBuffer = ref.buffer.slice(position, position+length)
      buffer.set(new Uint8Array(sourceBuffer), offset)
